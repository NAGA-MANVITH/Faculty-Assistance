        ...
        if(response == "Null") {
            ...
            while(index < responseArray.length) {
                if(responseArray[index] == '') {
                    responseArray[index] = 'Not attained';
                }
    ...
    $.each(rollNumbersGlobal, function(index, number) {
        for(var i = 1; i <= sections; i++) {
            ...
            for(var r = 1; r <= attemptQuestions; r++) {
                ...
                if(maxX > 0) {
                    var courseOutcome = document.getElementById('co'+i+'.'+index).value;
                    courseOutcomeObject['co'+courseOutcome].attempted += 1;
                }
                questionMarks[index] = 0;
                if((maxX / qMarks) * 100 >= 40) {
...
function submitData() {
    ...
    $.get('Servlet6', {
        ...
        window.location = "http://localhost:8081/Main_Project/home.jsp";
    });
...
function validateMarks(student) {
    ...
    for(var i = 1; i <= sections; i++) {
        ...
        for(var j = 1; j <= questions; j++) {
            ...
            for(var k = 0; k < bits; k++) {
                var strTemp = '' + i + '.' + j + '.' + alphabets[k];
                var bitMarks = parseInt(document.getElementById(student+'-'+strTemp).value);
            ...
            document.getElementById('totalMarks'+student).value = totalMarksTemp;
    }
}
        ...
        $scope.generateUI = function(schema) {
            ...
            for(var r = 1; r <= sections; r++) {
                ...
                for(var s = 1; s <= questions; s++) {
                    ...
                    table += '<td colspan=' + bits + '><input type="textfield" onchange="' + dummyString +'" class="form-control" id="co' + r + '.' + s + '"></td>';
                }
            }
            table += '</tr>';
            ...
            $.get('Servlet5', {
                ...
                $.each(response, function(index, value) {
                    ...
                    tableTemp += '<tr id="tr' + value + '"><td>'+value+'</td><td><input type="checkbox" id="'+value+'" class="form-control" name="pa" onclick="'+ stringTemp +'"></td>';
                    for(var i = 0; i < count; i++) {
                        tempString = "validateMarks('" + value + "')";
                        tableTemp += '<td><input type="textbox" onchange="' + tempString + '" class="form-control" id="' + value + '-' + questionNames[i] + '"></td>';
                    }
                    tableTemp += '<td><input type="textbox" class="form-control" id="totalMarks'+value+'" disabled></td></tr>';
                })
                //console.log(tableTemp);
                
                document.getElementById('tableDivision').innerHTML = '<table border=2 class="w3-table w3-striped myTable">' + table+tableTemp + '</table><div class="row"><div class="col-md-4"><button style="display:none" class="btn btn-primary" id="saveData" onclick="saveData()" type="submit">Save</button></div><div class="col-md-4"><button class="btn btn-primary" id="submitData" onclick="submitData()" type="submit">Submit</button></div></div>';
            });
        }
